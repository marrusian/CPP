...
inline bool is_separator(const std::string::value_type& x)
{
...
int main(void)
{
    ...
    std::map<CreditCards,std::string> ccnames
    {
    ...
    const size_t CCN_REGEXPS = ccnames.size();
    ...
    */
    ...
    if(!fin) throw std::runtime_error("Cannot open PAN input file.");
    if(fin.peek()==EOF) throw std::runtime_error("PAN input file is empty.");

    std::vector<std::regex> pans(ccnames.size());
    size_t p{};
    for(std::string temp; std::getline(fin,temp); ++p)
        pans[p] = temp;
    fin.close();
    ...
    for(std::string line; std::getline(fin,line); ++linenum){
        ...
        if(std::regex_search(line, generic_match, GenericCCN)){
            for(i = 0; i < CCN_REGEXPS && specific_match.empty(); ++i)
                std::regex_search(generic_match.str(), specific_match, pans[i]);

            std::cout << ccnames[static_cast<CreditCards>(i-1)] << ": " << specific_match[0];
            line = specific_match.str();
            if(luhn(line.begin(),line.end()))
                std::cout << " is valid.\n";
            else
